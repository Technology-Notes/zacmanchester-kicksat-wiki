...
If you are unable to install Ubuntu, then you can run the radio stuff inside a virtual machine on Windows or MacOS X. This easy to set up, but does not perform as well as a native installation. [Here are instructions for setting up the GNURadio inside VMWare Player on Windows](https://github.com/zacinaction/kicksat/wiki/Installing-GNURadio-(VMWare)).
